...
\textbf{Estimated total time:} 55 - 60 minutes
...
\section{Familiarizing with the Lab Environment}
\textbf{Estimated time:} 10 - 15 minutes

Before beginning the covert communication analysis, take a few minutes to explore the lab environment and verify that the virtual network is working correctly.
This warm-up task helps you understand how the Agent and DNS-DHCP VMs interact and gives you freedom to experiment safely - if anything breaks, simply rerun
\texttt{launch-lab} to reset the setup.

\subsection{Connecting to the Virtual Machines}
...
\textit{Goal:} Two terminals are open, one for the DNS Server and one for the Agent.

\subsection{Testing Network Connectivity}

From the \textbf{Agent VM}, confirm that the DNS Server is reachable:
\begin{lstlisting}[language=bash]
ping 192.168.10.1
\end{lstlisting}

From the \textbf{DNS Server}, list its network interfaces:
\begin{lstlisting}[language=bash]
ip addr
\end{lstlisting}

\textit{Goal:} The Agent should receive replies from 192.168.10.1, confirming basic connectivity.

\subsection{Verifying DNS Resolution}

From the \textbf{Agent VM}, perform a few DNS lookups through the server:
\begin{lstlisting}[language=bash]
nslookup example.com 192.168.10.1
dig example.com @192.168.10.1
\end{lstlisting}

\textit{Goal:} Both commands should return a valid IP address, confirming that \texttt{dnsmasq} is active on the DNS Server.

\subsection{Exploring Configuration and Services}
...
On the \textbf{DNS Server}:
\begin{lstlisting}[language=bash]
sudo systemctl status dnsmasq
\end{lstlisting}

\textit{Goal:} Identify which interface is used for communication, confirm the configured DNS resolver, and verify that \texttt{dnsmasq} is running.

\subsection{Breaking and Fixing the Network (Experiment)}

On the \textbf{DNS Server}, temporarily stop the DNS service:
\begin{lstlisting}[language=bash]
sudo systemctl stop dnsmasq
\end{lstlisting}

From the \textbf{Agent VM}, try another DNS lookup -- it should now fail.
Then restart the service:
\begin{lstlisting}[language=bash]
sudo systemctl start dnsmasq
\end{lstlisting}

\textit{Goal:} Observe how service availability affects network behavior and learn what happens when the server stops responding.
...
\subsection{Static IP Conflict Experiment}
\textbf{Note:} If there is aprox. 45 minutes of class left, feel free to skip this step as it is time consuming.


On the \textbf{Agent VM}, try assigning the same IP address as the DNS Server to the \texttt{eth1} interface:
\begin{lstlisting}[language=bash]
...
\textit{Observation:} The lookup will fail because both machines now claim the same IP address, creating a network conflict that prevents the real DNS Server from receiving queries.

\subsection{Reflection Questions}

\textbf{Question 1.7.1:} What IP address is assigned to the DNS Server, and how does the Agent obtain its IP address under normal conditions?

\textbf{Question 1.7.2:} Which service handles DNS requests on the server?

\textbf{Question 1.7.3:} After stopping \texttt{dnsmasq}, how did the Agent's behavior change?

\textbf{Question 1.7.4:} What happened when the Agent used the same IP address as the DNS Server?

\textbf{Question 1.7.5:} Why is it important to experiment with network failures before beginning covert communication analysis?

\newpage

\section{Single-Message Subdomain Encoding}
...
\begin{itemize}[noitemsep]
  ...
  \item You can also \textbf{save captures to pcap files} for later analysis or submission
...
This lab provides ready-to-use commands for running the DNS covert channel agent and decoder tool. You will not need to write code from scratch. However, one task includes a small coding exercise to help you understand the encoding mechanisms better. The codebase includes a README file with detailed usage instructions for both the agent and decoder tool if you need to reference them during the lab.

\subsection{Understanding Normal DNS Traffic}
...
\textbf{Question 2.1.1:} Count how many DNS query lines appeared in your tshark output. How many DNS queries were captured?

\textbf{Question 2.1.2:} What do normal DNS subdomains look like? Are they human-readable?

\subsection{Capturing Covert Channel Traffic}
...
\begin{enumerate}[noitemsep]
  ...
  \item Compare these subdomains to the normal ones from Task 2.1
  \item Stop the capture with Ctrl+C when done
\end{enumerate}

\textbf{Question 2.2.1:} What differences do you notice between normal DNS queries and covert channel queries? How do the subdomains look different?

\textbf{Question 2.2.2:} How many covert DNS query lines appeared in your tshark output?

\subsection{Identifying Encoding Types}
...
\textbf{Question 2.3.1:} Imagine looking at a captured covert traffic where a subdomain starts with "ob4xa". Based on the character set hints above, what encoding do you think this is? List the unique characters you see and explain your reasoning.

\subsection{Decoding Covert Messages}
...
\textbf{Question 2.4.1:} Now find and decode the "World" message from your tshark capture. First, extract the hex-encoded subdomain (remove the ".example.com" part), then decode it:
\begin{lstlisting}[language=bash]
...
\textbf{Question 2.4.2:} Decode the "Hello" message. Extract the base32 subdomain from your tshark capture and decode it. What subdomain did you find, and what did it decode to?

\subsection{Saving Captures to Pcap Files (For Submission)}
...
\textbf{Question 2.5.1:} Create a pcap file capturing your own covert messages. Save it as \texttt{task\_submission.pcap} and transfer it to your host machine. How many DNS query packets (requests only) are in your file? Use this command to count:
\begin{lstlisting}[language=bash]
sudo tshark -r /tmp/task_submission.pcap \
...
\subsection{Generating Encoded Messages}

Now that you understand how to decode messages, let's learn how to generate encoded messages without sending them.

\textbf{Question 2.6.1:} You need to exfiltrate the message "SecretData" using base32 encoding. What would the subdomain look like?
...
\begin{lstlisting}[language=bash]
python3 main.py --channel base32 --data "CovertBase" --mode send
\end{lstlisting}
  \item Observe the subdomain in the tshark output
\end{enumerate}

\textbf{Question 2.6.2:} What is the base32-encoded subdomain for "CovertBase"? Verify by decoding it - does it decode back correctly?

\subsection{Code Completion Exercise}
...
\textbf{Question 2.7.1:} If you encode the same message, with "base32" instead of "base32-exercise", what subdomain do you get? Does it match your implementation?

\textbf{Question 2.7.2:} After completing the function successfully, encode the message "Covert" using your function. What is the encoded output?

\textbf{Question 2.7.3:} Verify your encoded "Covert" message by decoding it with the decoder tool. Does it decode back to "Covert" correctly? You can use mode \texttt{print} to avoid sending and just see the encoded output.

\subsection{XOR Encryption Layer (Bonus)}
...
\textbf{Question 2.8.1 (Bonus):} Send a message using XOR encryption with your own secret key. What subdomain is generated? Specify also the key

\textbf{Question 2.8.2 (Bonus):} Try to decode it without the key using base32. What do you see?

\textbf{Question 2.8.3 (Bonus):} Decode it with the correct key. Does it work?

\textbf{Question 2.8.4 (Bonus):} Why is XOR encryption useful for covert channels? What additional security does it provide?

\newpage

\section{Multi-Packet Covert Channels}
...
\subsection{Understanding the RD Flag Channel}
...
\textbf{Question 3.1.1:} How many DNS queries are needed to transmit the 2-byte message "Hi"? Show your calculation.

\textbf{Question 3.1.2:} If you observe 64 DNS queries with varying RD flags, how many bytes of data were transmitted?

\subsection{Sending and Capturing RD Flag Messages}
...
\textbf{Question 3.2.1:} Looking at your tshark output, record the sequence of RD flag values for the "OK" message (should be 16 values: 8 bits for 'O' + 8 bits for 'K'). Write them as comma-separated values.

\subsection{Decoding Your Multi-Packet Messages}
...
\textbf{Question 3.3.1:} Now decode the "OK" message using the RD flag sequence you captured. Paste the RD sequence you use and confirm it decodes correctly to "OK".

\textbf{Question 3.3.2:} For verification, manually decode this RD flag sequence. What message was transmitted?
\begin{lstlisting}
RD:0,1,0,0,0,0,0,1
\end{lstlisting}
...
\subsection{Other Multi-Packet Channels}
...
\textbf{Question 3.4.1:} The TXID channel encodes data in the DNS transaction ID field (16 bits per query). How many queries are needed to transmit a 10-byte message? Show your calculation.

\textbf{Hint:} Each query has a 16-bit (2-byte) transaction ID.

\subsection{Testing Another Multi-Packet Channel}
...
\textbf{Question 3.5.1:} Based on your capture, record the Transaction ID values you see. How many queries were needed for the 4-byte message "Test"? Does this match your expectation (4 bytes ÷ 2 bytes/query = 2 queries)?
...
\textbf{Question 3.5.2:} Calculate: If you need to exfiltrate a 100-byte document, how many DNS queries would you need using:
\begin{itemize}[noitemsep]
...
\subsection{Complete Workflow Verification (Bonus)}
...
\textbf{Question 3.6.1 (Bonus):} Document your complete workflow:
\begin{itemize}[noitemsep]
