...
\section{Task 1: Familiarizing with the Lab Environment}
...
\textbf{Question 1.1:} What IP address is assigned to the DNS Server, and how does the Agent obtain its IP address under normal conditions?

\textbf{Question 1.2:} Which service handles DNS requests on the server?

\textbf{Question 1.3:} After stopping \texttt{dnsmasq}, how did the Agent's behavior change?

\textbf{Question 1.4:} What happened when the Agent used the same IP address as the DNS Server?

\textbf{Question 1.5:} Why is it important to experiment with network failures before beginning covert communication analysis?

\newpage

\section{Task 2: Single-Message Subdomain Encoding}
...
\textbf{Question 2.1a:} Count how many DNS query lines appeared in your tshark output. How many DNS queries were captured?

\textbf{Question 2.1b:} What do normal DNS subdomains look like? Are they human-readable?
...
\textbf{Question 2.2a:} What differences do you notice between normal DNS queries and covert channel queries? How do the subdomains look different?

\textbf{Question 2.2b:} How many covert DNS query lines appeared in your tshark output?
...
\textbf{Question 2.3a:} Imagine looking at a captured covert traffic where a subdomain starts with "ob4xa". Based on the character set hints above, what encoding do you think this is? List the unique characters you see and explain your reasoning.
...
\textbf{Question 2.4a:} Now find and decode the "World" message from your tshark capture. First, extract the hex-encoded subdomain (remove the ".example.com" part), then decode it:
\begin{lstlisting}[language=bash]
...
\textbf{Question 2.4b:} Decode the "Hello" message. Extract the base32 subdomain from your tshark capture and decode it. What subdomain did you find, and what did it decode to?
...
\textbf{Question 2.4c:} Create a pcap file capturing your own covert messages. Save it as \texttt{task1\_submission.pcap} and transfer it to your host machine. How many DNS query packets (requests only) are in your file? Use this command to count:
\begin{lstlisting}[language=bash]
...
\textbf{Question 2.5a:} You need to exfiltrate the message "SecretData" using base32 encoding. What would the subdomain look like?
...
\textbf{Question 2.5b:} What is the base32-encoded subdomain for "SecretData"? Verify by decoding it - does it decode back correctly?
...
\textbf{Question 2.6a:} If you encode the same message, with "base32" instead of "base32-exercise", what subdomain do you get? Does it match your implementation?

\textbf{Question 2.6b:} After completing the function successfully, encode the message "Covert" using your function. What is the encoded output?

\textbf{Question 2.6c:} Verify your encoded "Covert" message by decoding it with the decoder tool. Does it decode back to "Covert" correctly? You can use mode \texttt{print} to avoid sending and just see the encoded output.
...
\section{Task 3: Multi-Packet Covert Channels}
...
\textbf{Question 3.1:} How many DNS queries are needed to transmit the 2-byte message "Hi"? Show your calculation.

\textbf{Question 3.2:} If you observe 64 DNS queries with varying RD flags, how many bytes of data were transmitted?
...
\textbf{Question 3.3:} Looking at your tshark output, record the sequence of RD flag values for the "Hi" message (should be 16 values: 8 bits for 'H' + 8 bits for 'i'). Write them as comma-separated values.
...
\textbf{Question 3.4:} Now decode the "OK" message using the RD flag sequence you captured. Paste the RD sequence you use and confirm it decodes correctly to "OK".

\textbf{Question 3.4b:} For verification, manually decode this RD flag sequence. What message was transmitted?
\begin{lstlisting}
RD:0,1,0,0,0,0,0,1
\end{lstlisting}
...
\textbf{Question 3.5:} The TXID channel encodes data in the DNS transaction ID field (16 bits per query). How many queries are needed to transmit a 10-byte message? Show your calculation.
...
\textbf{Question 3.6:} Based on your capture, record the Transaction ID values you see. How many queries were needed for the 4-byte message "Test"? Does this match your expectation (4 bytes ÷ 2 bytes/query = 2 queries)?
...
\textbf{Question 3.7:} Calculate: If you need to exfiltrate a 100-byte document, how many DNS queries would you need using:
\begin{itemize}[noitemsep]
...
\textbf{Question 3.8 (Bonus):} Document your complete workflow:
\begin{itemize}[noitemsep]
