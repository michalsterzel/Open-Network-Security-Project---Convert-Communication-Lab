...
DNS is a very good vector of attack because DNS traffic is ubiquitous and trusted meaning there is usually not a lot of control done but also because it is quite easy to add allows arbitrary data through different means explain bellow.    
...
DHCP automates network configuration of hosts/client. It makes the network aware of a new host and gives the host the configuration needed to communicate: assigning IP addresses, default gateway, DNS servers, and others.
The DHCP service is responsible for it's own subdomain. It is in control of a range of IP (EX:  [192.168.1.100,192.168.1.200]) However only 1 service should be in control of the incoming host, otherwise the host would be given conflicting information and cause overall network connectivity issues.
...
The stealth level is exceptionally high because DNS servers treat the queries identically regardless of case, and case variation in domain names appears completely natural. Very few security systems monitor character case in DNS queries, making this one of the most covert channels available. The channel works reliably through all DNS infrastructure since case preservation is standard behavior.

\subsection{Multi-Channel Combination}

Advanced adversaries sometimes combine multiple covert channels within single DNS queries to maximize data throughput. For example, a query might simultaneously use case toggling in the subdomain label, vary the label count, set a specific TXID value, and manipulate the RD flag. These channels operate independently, so the total bandwidth theoretically equals the sum of individual channel bandwidths.
...
ML advantages include automation at scale, complex pattern recognition, adaptability through retraining, and novel threat detection. Challenges include obtaining large labeled datasets, managing false positives, computational costs, adversarial evasion, model staleness, explainability difficulties, baseline drift, and cold start issues. Best practice combines rule-based detection for known threats, ML for anomalies, and human analysts providing continuous feedback.
