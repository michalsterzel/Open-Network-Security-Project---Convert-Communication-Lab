...
A DNS attack is an exploit in which a malicious actor takes advantage of vulnerabilities in the Domain Name System (DNS) to disrupt services, redirect users to malicious websites, or gain unauthorized access to an organization's resources. 

This lab focuses on covert channels: techniques to establish secret, unauthorized communication links between compromised systems and attacker servers. Attackers exploit the fact that DNS traffic is generally trusted and rarely inspected closely by firewalls or other security measures, allowing them to hide their malicious activities.

DNS is a particularly good vector for this type of attack because DNS traffic is ubiquitous and trusted, meaning it is usually subject to limited inspection. Moreover, it allows arbitrary data to be transmitted through several mechanisms, making it well-suited for covert communication.

\section{DNS (Domain Name System)}
\subsection{What is DNS?}
DNS, or the Domain Name System, is the Internet's phone book. It translates human-friendly domain names like \texttt{www.google.com} into machine-readable IP addresses (e.g., 172.217.160.142). This allows users to visit websites by typing an easy-to-remember name instead of a long string of numbers, which computers use to locate and connect to each other on a network. 

There are four main DNS components involved in resolving a domain name when loading a web page:
\begin{itemize}[noitemsep]
  \item \textbf{Root servers:} The root servers are the first step, and the most authoritative, in translating (resolving) human-readable host names into IP addresses. They can be thought of as a library that knows the names of every rack of books (the top-level delegations).
  \item \textbf{TLD name servers:} The top-level domain (TLD) servers can be thought of as a specific rack of books (for example, \texttt{.com}). A TLD nameserver is the next step in the search for a specific IP address and knows the authoritative nameservers for second-level domains.
  ...
  \item \textbf{Resolver:} The resolver acts like a librarian who is asked to find the translation in a particular book somewhere in the library. 
  There are two ways the resolver can search for answers:
  \begin{enumerate}
      \item \textbf{Recursive query:} The resolver asks a DNS server to find the final answer on its behalf (the resolver asks the library, which asks the rack, which asks the book - the answer then follows the same path back).
      \item \textbf{Iterative query:} The resolver performs each step itself: it asks the library which rack to choose, then asks the rack which book to use, and finally queries the book for the answer.  
  \end{enumerate}
\end{itemize}

\textbf{What about \texttt{hello.world.example.com}?}
\newline
As you may know, a URL can contain many fields. In this case, the \texttt{example.com} authoritative servers may be authoritative for the entire \texttt{example.com} zone (able to resolve every subdomain, even \texttt{hello.world.example.com}). Alternatively, the authority for the subdomain may delegate \texttt{world.example.com} to its own zone with its own authoritative servers.
...
A simplified recursive lookup works as follows:
\begin{enumerate}[noitemsep]
  \item The client asks its stub resolver (often the OS or a local resolver) for \texttt{host.example.com}.
  \item If the resolver does not have the answer, it queries root servers, then TLD servers, and finally the authoritative server for \texttt{example.com}.
  \item The authoritative server responds with the requested record(s).
\end{enumerate}

\subsection{Common DNS Record Types}
DNS records are the entries inside the ``dictionary.'' They store different types of information used by various services:
\begin{itemize}[noitemsep]
...
\end{itemize}
the resolver's goal is to ask for the correct record type to get the desired information ("I'm looking for the information behind the A field").

\section{DHCP (Dynamic Host Configuration Protocol)}
\subsection{What is DHCP?}
DHCP automates the network configuration of client hosts. It makes the network aware of a new host and provides the necessary configuration for communication: assigning IP addresses, default gateways, DNS servers, and other parameters. 

The DHCP service is responsible for its own address pool - it controls a specific range of IP addresses (for example, [192.168.1.100, 192.168.1.200]). However, only one DHCP service should operate on a network segment; otherwise, a host could receive conflicting configuration information, resulting in connectivity issues.

\subsection{Typical DHCP Flow}
\begin{enumerate}[noitemsep]
  \item \textbf{DHCPDISCOVER:} The client broadcasts a message to find available DHCP servers.
  \item \textbf{DHCPOFFER:} A server offers a lease containing an IP address and configuration options.
  \item \textbf{DHCPREQUEST:} The client requests the offered lease.
  \item \textbf{DHCPACK:} The server acknowledges and finalizes the lease.
\end{enumerate}
...
\end{tikzpicture}
\caption{DNS packet structure with exploitable fields highlighted in red: Transaction ID, Flags (RD bit), QNAME (subdomain, labels, case), QTYPE, and TTL (in responses). Blue outlines Header, green Question, gold Answer. These are the main channels tackled in this lab.}
