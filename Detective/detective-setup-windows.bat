@echo off
echo Setting up Vagrant Detective Lab...
echo.
...
REM Create Detective Vagrantfile with custom configuration
echo Step 2: Creating Detective Vagrantfile...
powershell -Command "@'
# -*- mode: ruby -*-
# vi: set ft=ruby :

# Detective VM Configuration
...
Vagrant.configure(\"2\") do |config|
  ...
  # This will put the detective on the same network as the DNS server (192.168.10.1)
  # The detective will get an IP from the DHCP server (192.168.10.3-192.168.10.100)
  ...
  config.vm.provider \"virtualbox\" do |vb|
    ...
    # Set VM name
    vb.name = \"detective-vm\"
  end

  # Provisioning script to configure the detective
  config.vm.provision \"shell\", inline: <<-SHELL
    ...
    echo \"Detective VM setup complete!\"
  SHELL
end
