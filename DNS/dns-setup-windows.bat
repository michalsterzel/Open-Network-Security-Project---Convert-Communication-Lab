...
powershell -Command "$content = Get-Content Vagrantfile; $index = 0; $newContent = @(); foreach($line in $content) { if($line -match 'config.vm.box = \"open_network_security/dns_server\"') { $newContent += $line; $newContent += '  config.vm.box_version = \"1.1.0\"' } else { $newContent += $line } }; $newContent | Set-Content Vagrantfile"

REM Configure internal network
echo Step 5: Configuring internal network...
powershell -Command "$content = Get-Content Vagrantfile; $index = 0; $newContent = @(); foreach($line in $content) { if($line -match 'config.vm.box_version = \"1.1.0\"') { $newContent += $line; $newContent += ''; $newContent += '  # Set up Internal Network for isolated VM-to-VM communication (VirtualBox specific)'; $newContent += '  # Assign a static IP address for this VM on the internal network'; $newContent += '  config.vm.network \"private_network\", ip: \"192.168.10.1\", virtualbox__intnet: \"labnet\"' } else { $newContent += $line } }; $newContent | Set-Content Vagrantfile"
